...
import { AbstractPacket, util } from '.';

const { decodeRawMeta, metaLength } = util;
...
export class BufferParser extends EventEmitter /* implements Parser */ {
  ...
  private acceptedTypes: number[];
  private packets: Map<number, typeof AbstractPacket>;
  ...
  decode(buf: Buffer): AbstractPacket {
    ...
    const PacketClass = this.packets.get(type);
